...
(function($){
    $.fn.deSerializeObject = function (obj) {
        this.find("select,textarea,input[type!='radio'][type!='checkbox']").each(function(idx,e){
            if(e.name)
                $(e).val(obj[e.name] == undefined ? "" : obj[e.name] );
        });
...
function searchGrid(e){
    ...
    $form.find(".datetimepickerspan").each(function(i,e){
        var $this = $(e);
        var startTime = $this.find(".startTime").val().toDate().getTime();
        var endTime = $this.find(".endTime").val().toDate().getTime();
...
$(window).resize(function(e){
    ...
    var size = getComputedStyle($('body')[0]);
    var width = parseInt(size['width'].replace("px","")) -  ($splitLine.hasClass('menu-show') ? 245 : 15);
    var height = document.documentElement.clientHeight-80; //footer and header

    $content.css("width",width+"px");
    $sideMenu.css('height',height+"px");
    $content.css('height',height+'px');
    $splitLine.css('height',height+"px");
    //调整页面表单大小
